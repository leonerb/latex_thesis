...
\begin{figure}[h]
    ...
    \caption{\textbf{Label extraction and hierarchical structure of the CheXpert dataset.} 
	Labels are grouped into 14 categories.
...
\begin{figure}[ht]
	...
	A single pair of views $(x_1,x_2)$ is assumed to simplify the illustration \citep{Caron2021}.
	The exponential moving average is abbreviated as EMA.
	The crossed out gradient $\cancel{\nabla}$ indicates a gradient backpropagation for the student weights but not for the teacher weights.
	}
	\label{fig:dino-arch}
\end{figure}
%</Figure:dino-arch>
...
\begin{figure}
	\begin{subtable}[c]{\textwidth}
		...
		\\[-3pt]
		\midrule
		Cardiomegaly&57.8&55.3&58.2&\textbf{60.3}\\
		Fracture&53.4&54.5&\textbf{56.4}&55.7\\
		Lung Opacity&55.6&55.5&59.1&\textbf{60.3}\\
	...
	AUROC values are based on the feature similarity score and maximum values are shown in bold.
	...
	The dotted line refers to the chance level performance at 50\% AUROC.
	For the ImageNet normalization, all images were scaled with $\mu=(0.485, 0.456, 0.406)$ and $\sigma=(0.229, 0.224, 0.225)$. 
    The CheXpert normalization stands for a normalization with $\mu=(0.5330, 0.5330, 0.5330)$ and $\sigma=(0.0349, 0.0349, 0.0349)$ \citep[\href{https://github.com/christophbrgr/ood_detection_framework}{\color{Periwinkle}{implementation}}]{Berger2021}.
...
\begin{figure}[bht!]
	...
	The values for setting 1 (ID: \textit{No Finding}) are calculated as a top-3 average over \textit{Pleural Effusion}, \textit{Cardiomegaly} and \textit{Pneumothorax}.
...
\begin{figure}[h]
	...
	\caption{\textbf{Comparison of supervised (Sup ViT, Berger et al. \citep{Berger2021}) and unsupervised (ViT DINO pretrained) models for setting 2 and setting 3.}
	The ID classes for setting 2 are \textit{Cardiomegaly} and \textit{Pneumothorax} and the OOD class is \textit{Fracture}.
	The ID classes for setting 3 are \textit{Lung Opacity} and \textit{Pleural Effusion} and the OOD classes are \textit{Fracture} and \textit{Pneumonia}.}
...
\begin{figure}[!h]
	...
	The green bounding boxes represent the limit values.
	Within the green bounding boxes, possible slices are sampled and augmentations are applied to those slices (red bounding boxes).
	The simulated \textit{Fracture} is located in the upper part of the right side of the image (left side of the patient) and is marked with a blue arrow.}
...
\begin{figure}[h]
	...
	Supervised outlier exposure requires access to fine-grained OOD labels and unsupervised outlier exposure does not (see \ref{section: Outlier Exposure}).}
