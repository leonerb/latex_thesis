...
\begin{figure}[htbp]
	...
	\begin{subfigure}[t]{0.3\textwidth}
		...
		%\label{fig:sixth}
	\end{subfigure}	
	\caption{Pathologies: Pictures are taken from the CheXpert dataset \citep{Irvin2019}.}
...
\begin{figure}[ht]
	...
	\caption{Model architecture of ViT Transformer. Adapted from \citep{Dosovitskiy2020}. 
...
\begin{figure}[ht]
	...
	The crossed out gradient $\cancel{\nabla}$ indicates, that there is a gradient back propagation for the student- but not for the teacher weights.
	}
	\label{fig:dino-arch}
\end{figure}
%</Figure:dino-arch>
...
\begin{figure}[!htbp]
	...
	AUROC values are based on the feature similarity and maximum values are marked in bold.
...
\begin{figure}[ht]
	...
	\caption{Comparison of the adapted DINO augmentations and additionally adding a rotation by 20 degrees (Rotate augmentations, detailed in \ref{section: adapted-methods}).}
...
\begin{figure}[ht]
	...
	\caption{Comparison of supervised (Sup ViT, Sup. Baseline, ODIN) and unsupervised models (DINO) for setting 2 and setting 3.}
...
\begin{figure}[!htbp]
	...
	The red bounding box marks the region of interest.}
...
\begin{figure}[h]
	...
	\caption{Outlier exposure on Setting 1: OoDD performance between Pleural Effusion, Cardiomegaly, Pneumothorax (OOD) against No Finding (ID).
	The average is calculated over the OOD classes to compare with \ref{fig:setting1-chexnorm-v-imgnorm-dino-last-epoch}.
...
\begin{figure}[!htbp]
	...
	\caption{Outlier exposure on setting 1: OoDD performance between Fracture (OOD) and No Finding (ID).
	Supervised outlier exposure assumes access to fine-grained OOD labels and unsupervised outlier exposure does not (see \ref{section: Out-of-Distribution Detection}).}
