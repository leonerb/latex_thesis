...
\begin{figure}[h]
	...
	The dotted arrow from the Support Devices class indicates, that patients with a support device can also be labelled as No Finding, representing the only source of overlap between the No Finding class and other classes.
...
\begin{figure}[h]
	...
	\caption{\textbf{Outlier exposure on Setting 1.} Average OoDD performance between No Finding (ID) and Pleural Effusion, Cardiomegaly, Pneumothorax (OOD).
...
\begin{figure}[!htbp]
	...
	\caption{\textbf{Outlier exposure on setting 1.} OoDD performance between No Finding (ID) and Fracture (OOD).
	Supervised outlier exposure assumes access to fine-grained OOD labels and unsupervised outlier exposure does not (see \ref{section: Out-of-Distribution Detection}).}
